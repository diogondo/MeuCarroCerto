...
namespace MeuCarroCerto.Repositorios
{
    ...
    {
        public static void RegistraCookieAutenticacao(long codigo)
        {
            //Criando um objeto cookie
            HttpCookie UserCookie = new HttpCookie("UserCookieAuthentication");

            //Setando o ID do usuário no cookie
            UserCookie.Values["codigo"] = MeuCarroCerto.Repositorios.RepositorioCriptografia.Criptografar(codigo.ToString());
